...
| 能力 | 说明 |
| --- | --- |
| 身份隔离 | Agent 只认系统注入的客户身份；别人账户的单号一律回答"没找到"，**不区分"不存在"和"不是你的"** |
...
3. 切到 **王强 (C1003)**，仍问订单 `O202609190902` → 回答「您的账户下没有找到该订单」，**商品名都不给**。
...
│   ├── code/order_action.py    代码节点：三道闸（数据边界 / 归属校验 / 状态机）
...
- **数据**：Python 3 生成器
- **Agent**：Dify Chatflow（开始 · 意图识别 · 路由 · HTTP 查订单 · 代码节点判定 · 回复生成，政策支路另接知识检索）
